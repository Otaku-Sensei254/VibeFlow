...
export default function Layout() {
  ...
  const badgeFor = (label) => {
    if (label === "Alerts") return notificationCount;
    if (label === "Chat") return chatUnreadCount;
    return 0;
  };
      ...
      <nav className="fixed top-0 z-50 w-full bg-white/90 dark:bg-gray-900/90 backdrop-blur border-b border-gray-200 dark:border-gray-700">
        <div className="max-w-7xl mx-auto px-4 h-14 flex items-center justify-between">
              ...
              return (
                ...
                >
                  <div className="relative">
                    <link.icon size={18} />
                    {count > 0 && (
                      <span className="absolute -top-1.5 -right-1.5 bg-coral-500 text-white text-[10px] font-bold min-w-[16px] h-4 flex items-center justify-center rounded-full px-1">
                        {count > 99 ? "99+" : count}
                      </span>
                    )}
                  </div>
          ...
          <div className="flex items-center gap-2">
            {user && (
              <div className="relative group hidden md:block" title={connectionStatus === "connected" ? "Live" : connectionStatus === "connecting" ? "Connecting..." : "Offline"}>
                <span className={`block w-2.5 h-2.5 rounded-full ring-2 ring-white dark:ring-gray-900 ${
              ...
              >
                {isProfile && user ? (
                  ...
                  <div className="relative">
                    <link.icon size={20} />
                    {count > 0 && (
                      <span className="absolute -top-1.5 -right-1.5 bg-coral-500 text-white text-[10px] font-bold min-w-[14px] h-3.5 flex items-center justify-center rounded-full px-0.5">
                        {count > 99 ? "99+" : count}
                      </span>
                    )}
                  </div>
